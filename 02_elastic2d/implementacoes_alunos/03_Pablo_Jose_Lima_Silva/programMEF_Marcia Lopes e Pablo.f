...
c
     ...
     .         a(i19),neq)
c
c     Calculo das tensoes nodais suavizadas para pos-processamento:
c       Metodo usado para Q4: tensoes nos pontos de Gauss 2x2,
c       extrapolacao das componentes de tensao para os nos do elemento,
c       media das contribuicoes dos elementos conectados ao mesmo no e,
c       por fim, calculo de Von Mises a partir das componentes medias.
c
      i20 = i19 + neq
      i21 = i20 + nnode
      i22 = i21 + nnode
      ...
      i26 = i25 + nnode
      call mem(i26)
      call vmnodal(a,m(i1),m(i2),m(i3),a(i4),a(i5),a(i6),
     .             a(i20),a(i21),a(i22),a(i23),a(i24),a(i25),
     .             nnode,numel,ndm,nen,ndf)
c
...
c
      ...
      call wvtk(m(i2),m(i3),a(i4),a(i5),a(i6),a(i20),a(i21),
     .          a(i22),a(i23),a(i24),nnode,numel,ndm,nen,ndf,nvtk)
      close(nvtk)
...
c ======================================================================
      subroutine vmnodal(e,ie,ix,id,x,f,u,vm,sxnod,synod,txynod,
     .                   sznod,cnt,nnode,numel,ndm,nen,ndf)
c     Calcula tensoes nodais suavizadas para pos-processamento.
c
c     Para quadrilateros Q4 (iel=3 ou 4), o procedimento segue a logica
c     usual de pos-processadores como PrePoMax/CalculiX para visualizacao
c     nodal suavizada:
c       1) calcula sigma_x, sigma_y, tau_xy e sigma_z nos 4 pontos de
c          Gauss do elemento;
c       2) extrapola cada componente dos pontos de Gauss para os 4 nos
c          do elemento;
c       3) faz a media aritmetica das contribuicoes de todos os elementos
c          conectados ao mesmo no;
c       4) calcula Von Mises no no usando as componentes medias.
c
c     Para triangulos T3 (iel=1 ou 2), a tensao e constante no elemento;
c     portanto, apenas se faz media nodal entre elementos conectados.
c ======================================================================
      integer nnode,numel,ndm,nen,ndf
      integer ie(*),ix(nen+1,*),id(ndf,*)
      real*8  e(10,*),x(ndm,*),f(ndf,*),u(*)
      real*8  vm(*),sxnod(*),synod(*),txynod(*),sznod(*),cnt(*)
      real*8  xl(2,4),ul(8),sx(4),sy(4),txy(4),sz(4)
      real*8  sxm,sym,txym,szm,arg
      integer i,j,k,nel,no,ma,iel
c
c     Inicializacao dos acumuladores nodais:
c
      do 10 i = 1, nnode
        vm(i)     = 0.d0
        sxnod(i)  = 0.d0
...
c
      do 300 nel = 1, numel
        ...
        iel = ie(ma)
c
c       Monta coordenadas locais e deslocamentos locais do elemento:
c
        do 80 i = 1, nen
          no = ix(i,nel)
          xl(1,i) = x(1,no)
   ...
   80   continue
c
c       Calcula tensoes nodais do elemento:
c
        if (nen .eq. 4 .and. (iel .eq. 3 .or. iel .eq. 4)) then
          call q4stressn(e(1,ma),xl,ul,sx,sy,txy,sz,iel,nel)
        else if (nen .eq. 3 .and. (iel .eq. 1 .or. iel .eq. 2)) then
          call t3stressn(e(1,ma),xl,ul,sx,sy,txy,sz,iel,nel)
        else
          goto 300
        endif
c
c       Acumula componentes extrapoladas/constantes nos nos globais:
c
        do 120 i = 1, nen
          no = ix(i,nel)
          sxnod(no)  = sxnod(no)  + sx(i)
          synod(no)  = synod(no)  + sy(i)
          txynod(no) = txynod(no) + txy(i)
          sznod(no)  = sznod(no)  + sz(i)
          cnt(no)    = cnt(no)    + 1.d0
  120   continue
  300 continue
c
c     Media nodal das componentes e calculo de Von Mises:
c
      do 400 i = 1, nnode
        if (cnt(i) .gt. 0.d0) then
          sxnod(i)  = sxnod(i)  / cnt(i)
          synod(i)  = synod(i)  / cnt(i)
          txynod(i) = txynod(i) / cnt(i)
          sznod(i)  = sznod(i)  / cnt(i)
        endif
        sxm  = sxnod(i)
        sym  = synod(i)
        txym = txynod(i)
        szm  = sznod(i)
        arg  = 0.5d0*((sxm-sym)**2 + (sym-szm)**2 +
     .                 (szm-sxm)**2) + 3.d0*txym**2
        if (arg .lt. 0.d0 .and. dabs(arg) .lt. 1.d-20) arg = 0.d0
        if (arg .ge. 0.d0) vm(i) = dsqrt(arg)
  400 continue
      return
      end

c ======================================================================
      subroutine q4stressn(e,x,u,sxnod,synod,txynod,sznod,iel,nel)
c     Tensoes nodais de um elemento Q4 por extrapolacao dos pontos de
c     Gauss 2x2 para os nos do elemento.
c ======================================================================
      integer iel,nel,i,k
      real*8  e(*),x(2,4),u(8),sxnod(4),synod(4),txynod(4),sznod(4)
      real*8  sxg(4),syg(4),txyg(4),szg(4),egp(4,4)
      real*8  xj(2,2),xji(2,2),hr(4),hs(4),hx(4),hy(4)
      real*8  rr(4),ss(4),g,det,epsx,epsy,gamxy
      real*8  d11,d12,d21,d22,d33,my,nu,a,b,c,alam,arg
c
c     Ordem dos pontos de Gauss abaixo: perto dos nos locais 1,2,3,4.
c     Nos naturais do Q4 deste codigo:
c       no 1=(+1,+1), no 2=(-1,+1), no 3=(-1,-1), no 4=(+1,-1)
c
      g = 0.577350269189626d0
      rr(1) =  g
      ss(1) =  g
      rr(2) = -g
      ss(2) =  g
      ...
      ss(4) = -g
c
c     Matriz de extrapolacao de Gauss para no: avalia o campo bilinear
c     ajustado nos pontos de Gauss nos cantos naturais (+/-1,+/-1).
c
      a = 1.d0 + dsqrt(3.d0)/2.d0
      b = -0.5d0
      c = 1.d0 - dsqrt(3.d0)/2.d0
      egp(1,1) = a
      egp(1,2) = b
      egp(1,3) = c
      egp(1,4) = b
...
c
      ...
      if (iel .eq. 3) then
        ...
        d22 = c
        d33 = my/(2.d0*a)
        alam = my*nu/((1.d0+nu)*(1.d0-2.d0*nu))
      else
        a   = my/(1.d0-nu*nu)
        d11 = a
        d12 = a*nu
...
c
c     Tensoes nos quatro pontos de Gauss:
c
      do 200 i = 1, 4
        hr(1) =   (1.d0+ss(i)) / 4.d0
        hr(2) = - (1.d0+ss(i)) / 4.d0
...
c
        ...
        gamxy = 0.d0
        do 120 k = 1, 4
          epsx  = epsx  + hx(k)*u((k-1)*2+1)
          epsy  = epsy  + hy(k)*u((k-1)*2+2)
          gamxy = gamxy + hy(k)*u((k-1)*2+1)
     .                  + hx(k)*u((k-1)*2+2)
  120   continue
        sxg(i)  = d11*epsx + d12*epsy
        syg(i)  = d21*epsx + d22*epsy
        txyg(i) = d33*gamxy
        if (iel .eq. 3) then
          szg(i) = alam*(epsx+epsy)
        else
          szg(i) = 0.d0
        endif
  200 continue
c
c     Extrapolacao das componentes para os quatro nos do elemento:
c
      do 300 i = 1, 4
        sxnod(i)  = 0.d0
        synod(i)  = 0.d0
        ...
        sznod(i)  = 0.d0
        do 250 k = 1, 4
          sxnod(i)  = sxnod(i)  + egp(i,k)*sxg(k)
          synod(i)  = synod(i)  + egp(i,k)*syg(k)
          txynod(i) = txynod(i) + egp(i,k)*txyg(k)
          sznod(i)  = sznod(i)  + egp(i,k)*szg(k)
  250   continue
  300 continue
      return
 1000 continue
      print*, '*** ERRO q4stressn: det nulo ou negativo no elemento', nel
      print*, '    Verifique a ordem dos nos (deve ser anti-horaria).'
      stop
      end

c ======================================================================
      subroutine t3stressn(e,x,u,sxnod,synod,txynod,sznod,iel,nel)
c     Tensao constante de um elemento T3. Usa media nodal posteriormente.
c ======================================================================
      integer iel,nel,i,k
      real*8  e(*),x(2,3),u(6),sxnod(4),synod(4),txynod(4),sznod(4)
      real*8  det,xji11,xji12,xji21,xji22,hx(3),hy(3)
      real*8  xj11,xj12,xj21,xj22,epsx,epsy,gamxy
      real*8  d11,d12,d21,d22,d33,my,nu,a,b,c,alam,sx,sy,txy,sz
c
      xj11 = x(1,1)-x(1,3)
      xj12 = x(2,1)-x(2,3)
      xj21 = x(1,2)-x(1,3)
...
c
  ...
  200 continue
      return
 1000 continue
      print*, '*** ERRO t3stressn: det nulo ou negativo no elemento', nel
      print*, '    Verifique a ordem dos nos (deve ser anti-horaria).'
      stop
      end

c ======================================================================
      subroutine wvtk(ix,id,x,f,u,vm,sxnod,synod,txynod,sznod,
...
c       - Conectividade dos elementos
c       - Dados nodais: deslocamentos e tensoes nodais suavizadas
c ======================================================================
      integer nnode,numel,ndm,nen,ndf,ix(nen+1,*),id(ndf,*),nvtk
      real*8  x(ndm,*),f(ndf,*),u(*),aux(3)
      real*8  vm(*),sxnod(*),synod(*),txynod(*),sznod(*)
      integer i,j,k
c
c     Cabecalho VTK:
c
...
c
      if (ndf .eq. 2) then
c
c       Dados nodais: deslocamentos e tensoes suavizadas.
c       O POINT_DATA aparece uma unica vez, e todos os campos nodais
c       ficam dentro dele. Assim o ParaView interpreta Von Mises como
c       campo nodal continuo, nao como tensao constante por elemento.
c
...
c
  ...
  400   continue
c
        write(nvtk,'(a)') 'SCALARS Von_Mises_Nodal float'
        write(nvtk,'(a)') 'LOOKUP_TABLE default'
        do 410 i = 1, nnode
          write(nvtk,'(1e16.8)') vm(i)
  410   continue
c
        write(nvtk,'(a)') 'SCALARS Sxx_Nodal float'
        write(nvtk,'(a)') 'LOOKUP_TABLE default'
        do 420 i = 1, nnode
          write(nvtk,'(1e16.8)') sxnod(i)
  420   continue
c
        write(nvtk,'(a)') 'SCALARS Syy_Nodal float'
        write(nvtk,'(a)') 'LOOKUP_TABLE default'
        do 430 i = 1, nnode
          write(nvtk,'(1e16.8)') synod(i)
  430   continue
c
        write(nvtk,'(a)') 'SCALARS Sxy_Nodal float'
        write(nvtk,'(a)') 'LOOKUP_TABLE default'
        do 440 i = 1, nnode
          write(nvtk,'(1e16.8)') txynod(i)
  440   continue
c
        write(nvtk,'(a)') 'SCALARS Szz_Nodal float'
        write(nvtk,'(a)') 'LOOKUP_TABLE default'
        do 450 i = 1, nnode
          write(nvtk,'(1e16.8)') sznod(i)
  450   continue
        return
